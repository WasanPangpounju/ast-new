    ...
    fetch(`/api/bill-of-structures?sourceOrderId=${id}`)
      ...
      .then(d => {
        ...
        if (Array.isArray(b.deadlines) && b.deadlines.length > 0) {
          setDeadlines(b.deadlines.map((dl: { dt: string | null; qty: number | null; unit: string | null; pct: number | null }) => ({
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-[#ece9d8] border-b-2 border-gray-400 px-2 pt-1 flex items-end justify-between select-none">
        <div className="flex items-end gap-0.5">
          <div className="px-5 py-1.5 text-sm border border-b-0 border-gray-400 font-medium bg-white text-gray-900 relative -mb-px z-10">
            🏭 ใบโครงสร้าง
          </div>
        </div>
        <div className="pb-1.5 pr-1 text-sm text-gray-600">
          วันที่ {thaiDate}&nbsp;&nbsp;NO.<span className="font-semibold text-blue-700">{purchaseOrder || '...'}</span>
        </div>
      </div>
            ...
            {secOpen && (
              <div className="p-5 space-y-4">
                ...
                <div className="flex justify-end text-sm text-gray-600">
                  วันที่ {thaiDate}&nbsp;&nbsp;NO.<span className="font-medium text-gray-900">{purchaseOrder || '...'}</span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input value={customerName} onChange={setCustomerName} placeholder="ชื่อลูกค้า" />
                  </div>
                  <div>
                ...
                <div className="flex justify-center gap-3 pt-4 border-t border-gray-100">
                  ...
                  <button type="button" onClick={() => router.back()}
                    className="px-7 py-2 bg-gray-200 text-gray-700 text-sm rounded font-medium hover:bg-gray-300">
                    ← กลับใบสั่งขาย
                  </button>
                </div>
              </div>

...
const emptyForm = { name: '', tax: '', address: '', tel: '', email: '', type: 'ซัพพลายเออร์' }
...
export default function SalesSuppliersPage() {
  ...
  const fetchSuppliers = useCallback(() => {
    ...
    fetch(`/api/sales/suppliers?${p}`)
      .then(r => r.json())
      .then(d => { setSuppliers(d.suppliers ?? []); setTotal(d.total ?? 0) })
  ...
  async function toggleExpand(supplier: Supplier) {
    if (expanded === supplier.id) { setExpanded(null); return }
    setExpanded(supplier.id)
    setCoordLoading(true)
    try {
      const res = await fetch(`/api/sales/suppliers/${supplier.id}`)
  ...
  function openEdit(s: Supplier) {
    setEditing(s)
    setForm({ name: s.name, tax: s.tax ?? '', address: s.address ?? '', tel: s.tel ?? '', email: s.email ?? '', type: s.type ?? 'ซัพพลายเออร์' })
  ...
  async function handleSave() {
    ...
    try {
      const url = modal === 'edit' ? `/api/sales/suppliers/${editing!.id}` : '/api/sales/suppliers'
  ...
  return (
    <div className="p-4 max-w-full">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-lg font-semibold text-gray-900">ข้อมูลซัพพลายเออร์</h1>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200 text-xs">
                ...
                <th className="text-left px-3 py-2.5 font-medium text-gray-600">ชื่อซัพพลายเออร์</th>
              ...
              {loading ? (
                ...
                <>
                  <tr key={s.id}
                    ...
                    <td className="px-3 py-2.5 text-xs font-mono text-gray-600">{s.id}</td>
                    <td className="px-3 py-2.5 font-medium text-gray-900">{s.name}</td>
                    <td className="px-3 py-2.5 text-xs text-gray-600 font-mono">{s.tax ?? '-'}</td>
                    <td className="px-3 py-2.5 text-xs text-gray-600">{s.tel ?? '-'}</td>
                    <td className="px-3 py-2.5">
                      <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-700">{s.type ?? '-'}</span>
                    </td>
                    <td className="px-3 py-2.5 text-center" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => openEdit(s)}
                          className="text-xs px-2 py-1 bg-gray-100 text-gray-700 rounded hover:bg-gray-200">แก้ไข</button>
                        <button onClick={() => handleDelete(s)}
                          className="text-xs px-2 py-1 bg-red-50 text-red-600 rounded hover:bg-red-100">ลบ</button>
                      </div>
                    </td>
                  </tr>
                  {expanded === s.id && (
                    <tr key={`expand-${s.id}`}>
                      <td colSpan={7} className="bg-blue-50/60 px-6 py-3 border-t border-blue-100">
                        <div className="text-xs text-gray-600 mb-2 font-medium">ผู้ประสานงาน (tax: {s.tax})</div>
                        {coordLoading ? (
                          ...
                          <table className="text-xs w-full max-w-xl">
                            ...
                            <tbody>{coordinators.map(c => (
                              <tr key={c.id}>
                                <td className="pr-4 py-1 font-medium text-gray-800">{c.name}</td>
                                <td className="pr-4 py-1 text-gray-600">{c.jobTitle ?? '-'}</td>
                                <td className="py-1 text-gray-600">{c.tel ?? '-'}</td>
                              </tr>
                            ))}</tbody>
                          </table>
                        )}
                        <div className="mt-2 text-xs text-gray-500">
                          ที่อยู่: {s.address ?? '-'} &nbsp;|&nbsp; อีเมล: {s.email ?? '-'}
                        </div>
                      </td>
                    </tr>
                  )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              <h2 className="font-semibold text-gray-900 text-sm">{modal === 'add' ? 'เพิ่มซัพพลายเออร์ใหม่' : 'แก้ไขข้อมูลซัพพลายเออร์'}</h2>

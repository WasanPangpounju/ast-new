      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <>
                  <tr key={s.id}
                    ...
                    <td className="px-3 py-2.5 text-xs text-gray-500 w-48 max-w-[192px]">
                      <span className="block truncate" title={s.address ?? ''}>{s.address ?? '-'}</span>
                    </td>
                    ...
                    <tr key={`expand-${s.id}`}>
                      <td colSpan={8} className="bg-blue-50/60 px-6 py-3 border-t border-blue-100">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs text-gray-600 font-medium">ผู้ประสานงาน (tax: {s.tax ?? '-'})</span>
                          <button
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 bg-gray-50">
            <p className="text-xs text-gray-500">หน้า {page} จาก {totalPages} ({total.toLocaleString()} ราย)</p>
            <div className="flex gap-1">

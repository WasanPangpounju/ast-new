...
export default function StockCreateForm({ emp }: Props) {
  ...
  const [fabricCodeResults, setFabricCodeResults] = useState<
    FabricCodeOption[]
  >([]);
  ...
  useEffect(() => {
    if (!fabricCode) {
      setFabricCodeResults([]);
      return;
    }
    const t = setTimeout(() => {
      fetch(
        "/api/warehouse/stock/search?field=fabricCode&q=" +
          encodeURIComponent(fabricCode),
      )
        .then((r) => r.json())
  ...
  function handleAiResult(data: AiReadResult) {
    ...
    if (data.fabricCode != null) {
      setFabricCode(data.fabricCode);
      filled.add("fabricCode");
    }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  >
                    ...
                    <div className="text-gray-500 flex gap-3 mt-0.5">
                      ...
                      <span className="text-blue-600">
                        {Number(s.produced_fold).toLocaleString()} พับ
                      </span>
                    </div>
                  </button>
                ))}
          ...
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setFabricCodeDropdown(true);
              }}
              onFocus={() => {
                if (fabricCode) setFabricCodeDropdown(true);
              }}
              ...
              <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 shadow-lg max-h-48 overflow-y-auto">
                {fabricCodeResults.map((r, i) => (
                  <button
                    ...
                    type="button"
                    onMouseDown={() => {
                      setFabricCode(r.fabricCode);
                      setFabricCodeDropdown(false);
                    }}
                    className="w-full text-left px-3 py-2 hover:bg-blue-50 text-xs border-b border-gray-100 last:border-0"
                  >
                    ...
                    <div className="text-gray-500 mt-0.5">
                      {r.fabricStruct}
                      {r.fabricPattern ? " / " + r.fabricPattern : ""}
                    </div>
                  </button>
                ))}
              </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              onFocus={() => {
                if (customer) setCustomerDropdown(true);
              }}

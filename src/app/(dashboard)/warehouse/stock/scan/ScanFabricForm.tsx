...
export default function ScanFabricForm({ emp }: Props) {
  ...
  async function handleScan() {
    ...
    try {
      ...
      const res = await fetch("/api/ai/scan-fabric-sheets", {
        method: "POST",
        body: fd,
      });
    ...
    try {
      ...
      if (results.length === 0) {
        ...
        const exact = results.find(
          (r) => r.fabricCode?.toLowerCase() === code.trim().toLowerCase(),
        );
        if (exact) {
  ...
  async function handleSubmitAll() {
    const toSubmit = fabrics.filter(
      (f) => f.fabricStruct.trim() && parseRolls(f.rollsText).length > 0,
    );
    const skipped = fabrics.length - toSubmit.length;
    if (!toSubmit.length) {
      alert(
        "ไม่มีรายการที่พร้อมบันทึก (ต้องมีโครงสร้างผ้าและค่าพับอย่างน้อย 1 รายการ)",
      );
      return;
    }
    if (
      skipped > 0 &&
      !window.confirm(
        `มี ${skipped} รายการที่ข้อมูลไม่ครบ จะข้ามและบันทึกเฉพาะ ${toSubmit.length} รายการที่ครบ?`,
      )
    )
      return;
    ...
    for (const fabric of toSubmit) {
      try {
        ...
        const label =
          fabric.code ||
          fabric.customer ||
          `รายการที่ ${fabrics.indexOf(fabric) + 1}`;
        errors.push(
    ...
    if (errors.length) {
      alert(
        `บันทึกสำเร็จ ${totalCount} พับ\nผิดพลาด ${errors.length} รายการ:\n${errors.join("\n")}`,
      );
    } else {
      alert(
        `บันทึกสำเร็จทั้งหมด ${totalCount} พับ จาก ${toSubmit.length} รายการ`,
      );
      clearAll();
      setFabrics([]);
      setStep("upload");
      ...
      <div className="p-4 max-w-full">
        ...
        >
          ...
          <p className="text-xs text-gray-400 mt-1">
            รองรับ JPEG, PNG, WebP — อัปโหลดได้หลายรูปพร้อมกัน
          </p>
          <input
            ref={fileInputRef}
            type="file"
  ...
  const readyCount = fabrics.filter(
    (f) => f.fabricStruct.trim() && parseRolls(f.rollsText).length > 0,
  ).length;
  ...
  return (
    <div className="p-4 max-w-full">
          ...
          <div className="flex-1 flex items-end gap-3 flex-wrap">
            ...
            {imagePreviews.length > 0 && (
              ...
              >
                {showImages
                  ? "ซ่อนรูปต้นฉบับ"
                  : `แสดงรูปต้นฉบับ (${imagePreviews.length} รูป)`}
              </button>
            )}
          </div>
        </div>
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                <div>
                  ...
                  <select
                    value={fabric.section}
                    onChange={(e) =>
                      updateFabric(fabric.id, { section: e.target.value })
                    }
                ...
                <div>
                  ...
                  <input
                    value={fabric.customer}
                    onChange={(e) =>
                      updateFabric(fabric.id, { customer: e.target.value })
                    }
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="flex gap-1">
                    ...
                    />
                    <button
                      onClick={() => lookupFabric(fabric.id, fabric.code)}
                      disabled={
                        !fabric.code.trim() || fabric.lookupStatus === "loading"
                      }
                  ...
                  {fabric.lookupStatus === "not_found" && (
                    <p className="text-xs text-orange-500 mt-0.5">
                      ไม่พบรหัสนี้ — กรุณากรอกข้อมูลผ้าด้านล่างเอง
                    </p>
                  )}
                  {fabric.lookupStatus === "multiple" && (
                    <p className="text-xs text-blue-600 mt-0.5">
                      พบ {fabric.lookupOptions.length} รายการ —
              ...
              <div className="grid grid-cols-3 gap-3 mb-3">
                <div>
                  ...
                  <input
                    value={fabric.fabricStruct}
                    onChange={(e) =>
                      updateFabric(fabric.id, { fabricStruct: e.target.value })
                    }
                ...
                <div>
                  ...
                  <input
                    value={fabric.fabricPattern}
                    onChange={(e) =>
                      updateFabric(fabric.id, { fabricPattern: e.target.value })
                    }
                    placeholder="ลายผ้า"
                    className="w-full border border-gray-300 px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                ...
                <div>
                  ...
                  <input
                    value={fabric.fabricW}
                    onChange={(e) =>
                      updateFabric(fabric.id, { fabricW: e.target.value })
                    }
              ...
              <div>
                ...
                <textarea
                  value={fabric.rollsText}
                  onChange={(e) =>
                    updateFabric(fabric.id, { rollsText: e.target.value })
                  }
                  rows={2}
      ...
      <div className="flex items-center justify-between">
        <button
          onClick={() => setFabrics((prev) => [...prev, newRow()])}
          className="px-4 py-2 text-sm border border-gray-300 text-gray-600 hover:bg-gray-50"
        >
          + เพิ่มรายการ
        </button>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={submitting || !fabrics.length}
            className="px-6 py-2 text-sm bg-blue-600 text-white hover:bg-blue-700 font-medium disabled:opacity-50"
          >
